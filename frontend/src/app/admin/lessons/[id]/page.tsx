...
export default function AdminLessonEditPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([themesRes, lessonRes]) => {
      setThemes(themesRes.data as Theme[]);
      const d = lessonRes.data as {
        id: number; theme: number; title: string; order: number;
        lesson_type: string; xp_reward: number; stars_reward: number;
        content: Record<string, unknown> | null; starter_code: string;
        test_cases: unknown[];
      };
      setThemeId(d.theme);
      setForm({
        ...
        xp_reward: String(d.xp_reward),
        stars_reward: String(d.stars_reward),
        starter_code: d.starter_code ?? "",
        test_cases: d.test_cases ? JSON.stringify(d.test_cases, null, 2) : "[]",
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const resp = await api.patch(`/api/admin/lessons/${id}`, {
        ...
        test_cases: form.lesson_type === "practice" ? parsedTestCases : [],
      });
      ...
      <AdminHeader
        ...
        action={
          <div className="flex gap-2">
            ...
            {themeId && (
              <AdminBtn href={`/admin/themes/${themeId}/lessons`} variant="ghost">
                ← К урокам темы
              </AdminBtn>
            )}
          </div>
          ...
          <div className="flex flex-col lg:flex-row lg:h-[calc(100vh-73px)]">
            ...
            >
              ...
              <div className="flex flex-col gap-2 pt-2">
                ...
                {themeId ? (
                  <AdminBtn href={`/admin/themes/${themeId}/lessons`} variant="ghost">Отмена</AdminBtn>
                ) : (
